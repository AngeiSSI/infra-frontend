...
@Component({
  ...
  styles: [`
    ...
    .macro-tarea-header-simple {
      background: linear-gradient(135deg, #f5f5f5 0%, #eeeeee 100%);
      padding: 1rem 1.2rem;
      border-bottom: 2px solid #E0E0E0;
    }

    .macro-tarea-title-simple {
      display: flex;
      flex-direction: column;
      gap: 0.5rem;
    }

    .macro-tarea-nombre-simple {
      margin: 0;
      color: #1f2937;
      font-size: 1.05rem;
      font-weight: 800;
    ...
    .micro-tarea-row {
      display: grid;
      grid-template-columns: 50px 1fr 150px 100px 200px 150px 120px;
      gap: 1rem;
      padding: 1rem;
      align-items: center;
    ...
    .micro-tarea-observaciones {
      background: #f9fafb;
      padding: 1rem;
      border-top: 1px solid #e5e7eb;
    }

    .micro-tarea-observaciones h5 {
      margin: 0 0 1rem 0;
      color: #1f2937;
      font-size: 0.95rem;
      font-weight: 600;
    ...
    @media (max-width: 1200px) {
      .micro-tarea-row {
        grid-template-columns: 50px 1fr 100px 80px 150px 120px 100px;
        gap: 0.75rem;
        padding: 0.75rem;
      }
    }
...
export class ActividadesComponent implements OnInit {
  ...
  actualizarGruposMacroTareas(): void {
    // Seleccionar el método de agrupación según la vista
    if (this.vistaActual === 'mis') {
      // Mis actividades: agrupar por proyecto
      this.gruposMacroTareas = this.actividadesService.agruparPorProyecto(this.actividadesFiltradas);
    } else {
      // Grupo, Total, Seguimiento: agrupar por líder y proyecto
      this.gruposMacroTareas = this.actividadesService.agruparPorLiderYProyecto(this.actividadesFiltradas);
    }
...
crearActividad(): void {
  ...
  const nuevaActividad = {
    nombre: this.formData.actividadCatalogo,
    ...
    horas: Number(this.formData.horas),
    estado: 'en progreso',
    fechaCreacion: new Date(),
    fechaModificacion: new Date(),
    // ✅ Agregar macroTareaId para que se agrupe como micro tarea
    macroTareaId: `actividad-${Date.now()}`, // ID único temporal
    macroTareaNombre: 'Tareas Directas',
    indiceSecuencia: 0
  };

  console.log('📝 Creando actividad con:', {
    nombre: nuevaActividad.nombre,
    lider: nuevaActividad.lider,
    proyecto: nuevaActividad.proyecto,
    tipificacion: nuevaActividad.tipificacion,
  ...
  validarFormulario(): boolean {
    return !!(
      this.formData.proyecto && this.formData.proyecto.trim() &&
      this.formData.tipificacion && this.formData.tipificacion.trim() &&
      this.formData.actividadCatalogo && this.formData.actividadCatalogo.trim() &&
      this.formData.descripcion && this.formData.descripcion.trim() &&
      this.formData.horas && Number(this.formData.horas) > 0
    );
  }
  ...
  // Métodos helper para obtener datos agrupados
  obtenerLideresUnicos(): string[] {
    const lideresSet = new Set<string>();
    this.gruposMacroTareas.forEach(grupo => {
      if (grupo.lider) {
        lideresSet.add(grupo.lider);
      }
    });
    return Array.from(lideresSet).sort();
  }

  obtenerProyectosUnicos(): string[] {
    const proyectosSet = new Set<string>();
    this.gruposMacroTareas.forEach(grupo => {
      proyectosSet.add(grupo.proyecto);
    });
    return Array.from(proyectosSet).sort();
  }

  obtenerProyectosDelLider(lider: string): string[] {
    const proyectosSet = new Set<string>();
    this.gruposMacroTareas.forEach(grupo => {
  ...
  obtenerGruposDelLiderYProyecto(lider: string, proyecto: string): GrupoMacroTarea[] {
    return this.gruposMacroTareas.filter(
      grupo => grupo.lider === lider && grupo.proyecto === proyecto
    );
  }

  obtenerGruposDelProyecto(proyecto: string): GrupoMacroTarea[] {
    return this.gruposMacroTareas.filter(grupo => grupo.proyecto === proyecto);
  }
}